...
def set_object_transform_attributes(
    ...
    if not translate and not rotate and not scale:
        raise ValueError(
            "Must set at least one of the transform values (translate, rotate, or scale)."
        )
    ...
    if rotate and not _validate_vector3d(rotate):
        raise ValueError(
            "Invalid rotate format. Must be a list of 3 float values in degrees."
        )
    ...
    try:
        ...
        if rotate:
            cmds.setAttr(
                f"{object_name}.rotate", rotate[0], rotate[1], rotate[2], type="double3"
            )
        if scale:
            cmds.setAttr(
                f"{object_name}.scale", scale[0], scale[1], scale[2], type="double3"
            )
        results = {"success": True}
    except Exception as e:
        raise RuntimeError(
            f"Unable to update the transform attributes on object_name {object_name}"
        )
