...
def create_curve(
    ...
    if points and not all([_validate_vector3d(p) for p in points]):
        raise ValueError(
            "Invalid point format. Each point must be a list of 3 float values."
        )
    ...
    if curve_type.lower() == "custom":
        ...
        raise ValueError(
            f"Unknown curve type: {curve_type}. Use custom, line, circle, square, rectangle, spiral, helix, arc, star, or gear"
        )
    ...
    if parameters.get("periodic", False) and len(curve_points) > 3:
        ...
        curve = cmds.curve(
            name=name, p=curve_points, degree=min(degree, len(curve_points) - 1)
        )

    # Apply transformations
    cmds.setAttr(
        f"{curve}.translate", translate[0], translate[1], translate[2], type="double3"
    )
