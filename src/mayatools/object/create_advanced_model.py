...
def create_advanced_model(
    ...
    if not _validate_vector3d(color):
        raise ValueError(
            "Invalid color format. Must be a list of 3 float values between 0-1."
        )
    ...
    def finalize_model():
        ...
        cmds.setAttr(
            f"{model_group}.rotate", rotate[0], rotate[1], rotate[2], type="double3"
        )
    ...
    if model_type.lower() == "car":
        ...
        if not convertible:
            ...
            cabin = cmds.polyCube(
                w=cabin_width, h=cabin_height, d=2.0, name=f"{name}_cabin"
            )[0]
        ...
        for i, pos in enumerate(wheel_positions):
            wheel = cmds.polyCylinder(
                r=wheel_radius, h=wheel_width, ax=(1, 0, 0), name=f"{name}_wheel_{i}"
            )[0]
        ...
        if not convertible:
            # Windshield
            windshield = cmds.polyCube(w=1.4, h=0.5, d=0.1, name=f"{name}_windshield")[
                0
            ]
            ...
            # Rear window
            rear_window = cmds.polyCube(
                w=1.4, h=0.5, d=0.1, name=f"{name}_rear_window"
            )[0]
            ...
            # Side windows
            side_window_l = cmds.polyCube(
                w=0.1, h=0.4, d=1.4, name=f"{name}_side_window_l"
            )[0]
            side_window_r = cmds.polyCube(
                w=0.1, h=0.4, d=1.4, name=f"{name}_side_window_r"
            )[0]
        ...
        # Create headlights
        headlight_l = cmds.polyCylinder(
            r=0.2, h=0.1, ax=(1, 0, 0), name=f"{name}_headlight_l"
        )[0]
        headlight_r = cmds.polyCylinder(
            r=0.2, h=0.1, ax=(1, 0, 0), name=f"{name}_headlight_r"
        )[0]
        ...
        for comp in components:
            cmds.sets(
                comp,
                forceElement=cmds.sets(
                    name=f"{mat_name}SG", renderable=True, noSurfaceShader=True
                ),
            )
        ...
        # Create trunk
        trunk = cmds.polyCylinder(r=trunk_radius, h=trunk_height, name=f"{name}_trunk")[
            0
        ]
        ...
        trunk_mat = cmds.shadingNode(
            "lambert", asShader=True, name=f"{name}_trunk_material"
        )
        cmds.setAttr(f"{trunk_mat}.color", 0.3, 0.2, 0.1, type="double3")
        cmds.sets(
            trunk,
            forceElement=cmds.sets(
                name=f"{trunk_mat}SG", renderable=True, noSurfaceShader=True
            ),
        )
        ...
        if tree_type.lower() in ("oak", "maple", "deciduous"):
            ...
            for i in range(layers):
                ...
                cone = cmds.polyCone(
                    r=layer_radius, h=layer_height, name=f"{name}_foliage_{i}"
                )[0]
                y_pos = (
                    trunk_height
                    - (i * layer_height * 0.5)
                    + (layers * layer_height * 0.5)
                )
            ...
            trunk = cmds.polyCylinder(
                r=trunk_radius,
                h=trunk_height / segments,
                subdivisionsHeight=1,
                name=f"{name}_trunk_base",
            )[0]
            ...
            for i in range(1, segments):
                segment = cmds.polyCylinder(
                    r=trunk_radius * (1 - i * 0.05),
                    h=trunk_height / segments,
                    name=f"{name}_trunk_{i}",
                )[0]
                y_pos = trunk_height * (i + 0.5) / segments
                x_offset = curve_direction * (
                    math.sin(i / segments * math.pi * 0.5) * trunk_height * 0.15
                )
            ...
            for i in range(frond_count):
                ...
                frond = cmds.polyCone(
                    r=0.1, h=2.0, subdivisionsHeight=1, name=f"{name}_frond_{i}"
                )[0]
        ...
        leaf_mat = cmds.shadingNode(
            "lambert", asShader=True, name=f"{name}_leaf_material"
        )
        ...
        if foliage_components:
            cmds.sets(
                foliage_components,
                forceElement=cmds.sets(
                    name=f"{leaf_mat}SG", renderable=True, noSurfaceShader=True
                ),
            )
        ...
        # Create roof
        roof = cmds.polyCube(
            w=width * 1.1, h=height * 0.1, d=depth * 1.1, name=f"{name}_roof"
        )[0]
        ...
        for floor in range(floors):
            for i in range(windows_per_floor):
                ...
                window = cmds.polyCube(
                    ...
                    h=window_height,
                    d=0.1,
                    name=f"{name}_window_front_{floor}_{i}",
                )[0]
                ...
                window = cmds.polyCube(
                    ...
                    h=window_height,
                    d=0.1,
                    name=f"{name}_window_back_{floor}_{i}",
                )[0]
                cmds.move(window_x, window_y, -depth / 2, window)
                components.append(window)
                cmds.parent(window, model_group)
        ...
        for floor in range(floors):
            for i in range(windows_per_floor):
                ...
                window = cmds.polyCube(
                    ...
                    h=window_height,
                    d=window_width,
                    name=f"{name}_window_left_{floor}_{i}",
                )[0]
                ...
                window = cmds.polyCube(
                    ...
                    h=window_height,
                    d=window_width,
                    name=f"{name}_window_right_{floor}_{i}",
                )[0]
                cmds.move(width / 2, window_y, window_z, window)
                components.append(window)
                cmds.parent(window, model_group)
        ...
        # Apply materials
        building_mat = cmds.shadingNode(
            "lambert", asShader=True, name=f"{name}_building_material"
        )
        cmds.setAttr(
            f"{building_mat}.color", color[0], color[1], color[2], type="double3"
        )
        cmds.sets(
            [building, roof],
            forceElement=cmds.sets(
                name=f"{building_mat}SG", renderable=True, noSurfaceShader=True
            ),
        )

        # Window material
        window_mat = cmds.shadingNode(
            "lambert", asShader=True, name=f"{name}_window_material"
        )
        ...
        if window_objects:
            cmds.sets(
                window_objects,
                forceElement=cmds.sets(
                    name=f"{window_mat}SG", renderable=True, noSurfaceShader=True
                ),
            )

        # Door material
        door_mat = cmds.shadingNode(
            "lambert", asShader=True, name=f"{name}_door_material"
        )
        cmds.setAttr(f"{door_mat}.color", 0.4, 0.2, 0.1, type="double3")
        cmds.sets(
            door,
            forceElement=cmds.sets(
                name=f"{door_mat}SG", renderable=True, noSurfaceShader=True
            ),
        )
        ...
        inner = cmds.polyCylinder(r=inner_radius, h=inner_height, name=f"{name}_inner")[
            0
        ]
        ...
        if handle:
            ...
            handle = cmds.polyTorus(
                r=radius * 0.8, sr=radius * 0.1, twist=90, name=f"{name}_handle"
            )[0]
        ...
        raise ValueError(
            f"Error: unknown model type {model_type}, use one of these types: car, tree, building, cup, chair"
        )
