...
def create_material(
    ...
    if material_type.lower() == "lambert":
        ...
        # Set additional parameters
        for param, value in parameters.items():
            if param not in ["specularColor", "reflectivity"] and cmds.attributeQuery(
                param, node=shader, exists=True
            ):
                if isinstance(value, list) and len(value) == 3:
                    cmds.setAttr(
                        f"{shader}.{param}",
        ...
        for param, value in parameters.items():
            if param not in [
                ...
                "reflectivity",
                "eccentricity",
            ] and cmds.attributeQuery(param, node=shader, exists=True):
                if isinstance(value, list) and len(value) == 3:
                    cmds.setAttr(
                        f"{shader}.{param}",
                        value[0],
        ...
        if cmds.pluginInfo("mtoa", query=True, loaded=True):
            ...
            cmds.setAttr(
                f"{shader}.baseColor", color[0], color[1], color[2], type="double3"
            )
            ...
            for param, value in parameters.items():
                if param not in ["roughness"] and cmds.attributeQuery(
                    param, node=shader, exists=True
                ):
                    if isinstance(value, list) and len(value) == 3:
                        cmds.setAttr(
            ...
            cmds.setAttr(
                f"{shader}.color", color[0], color[1], color[2], type="double3"
            )
            cmds.setAttr(
                f"{shader}.specularColor", color[0], color[1], color[2], type="double3"
            )
        ...
        for attr in [
            ...
            if cmds.attributeQuery(
                attr, node=place2d, exists=True
            ) and cmds.attributeQuery(attr, node=wood_noise, exists=True):
                cmds.connectAttr(f"{place2d}.{attr}", f"{wood_noise}.{attr}")
        ...
        for attr in [
            ...
            if cmds.attributeQuery(
                attr, node=place2d, exists=True
            ) and cmds.attributeQuery(attr, node=marble_noise, exists=True):
                cmds.connectAttr(f"{place2d}.{attr}", f"{marble_noise}.{attr}")
        ...
        cmds.setAttr(
            f"{marble_noise}.color1", color[0], color[1], color[2], type="double3"
        )
        cmds.setAttr(
            f"{marble_noise}.color2",
            veinColor[0],
            veinColor[1],
            veinColor[2],
            type="double3",
        ...
        if cmds.pluginInfo("mtoa", query=True, loaded=True):
            ...
            if flakeIntensity > 0:
                flake_noise = cmds.shadingNode(
                    "aiNoise", asTexture=True, name=f"{name}_flakes"
                )
                ...
                color_mix = cmds.shadingNode(
                    "aiMixRgb", asUtility=True, name=f"{name}_color_mix"
                )
                created_nodes.append(color_mix)

                cmds.setAttr(
                    f"{color_mix}.input1", color[0], color[1], color[2], type="double3"
                )
                cmds.setAttr(
                    f"{color_mix}.input2",
                    flakeColor[0],
        ...
        raise ValueError(
            f"Unknown material type: {material_type}. Use lambert, phong, blinn, metal, wood, marble, chrome, glass, brushed_metal, or car_paint"
        )
